...
/*最佳适应法查找内存中第一个值为K的内存块*/
int BestSearch_SL(SLinkList space, ElemType K, int S); 
...
int main(int argc, char const* argv[])
{
	...
	printf("最佳适应法:从表头开始查找，当找到第一个满足要求的空闲区时\n");
	printf("           停止查找，并且这个空闲区是最佳的空闲区。\n\n");
...
int BestSearch_SL(SLinkList space, ElemType K, int S) 
{
	...
	for (i=space[S].cur; i<=volume; i=space[i].cur) {
		if (space[i].data>=K) {
			/*i-1是因为链表头结点下标为1,首元结点下标为2开始*/
			M=(space[i].data-K); /*保存合适的内存块和需要的内存之差*/
			/*如果更小,就替换Min值*/ 
			if (M<Min) {
				Min=M;
				Tag=i; /*并记录这个内存块的下标*/
	...
	/*做遍历后,如果找不到合适内存,输出相应提示*/
	if (Min==-1) {
		printf("找不到合适的内存.\n");
		return 0;
	} else {
		printf("\n找到合适的内存块,下标为:%d", Tag-1); 
		space[Tag].data=0;
